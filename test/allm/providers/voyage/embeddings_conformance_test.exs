defmodule ALLM.Providers.Voyage.EmbeddingsConformanceTest do
  @moduledoc """
  ...
  environment still observes the rejection rather than a
  ...
  **This suite does not bind `ALLM.EmbeddingAdapter` invariant 8 for this
  ...
  verbatim and never reaches `decode_response/4`, so a decoder that drops
  entries, mis-indexes them, or broadcasts one vector across every slot passes
  all ten cases. Invariant 8 is bound by the `decode_response/4` fixture test in
  `test/allm/providers/voyage/embeddings_test.exs`.
  """

  use ExUnit.Case, async: true
  use ALLM.Test.EmbeddingAdapterConformance, embedding_adapter: ALLM.Providers.Voyage.Embeddings
end
