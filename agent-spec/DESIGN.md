...
27. **Re-read every Test Plan bullet against the contract sections after locking them.** Test Plan bullets are derived artifacts and drift from their sources silently; a bullet that re-states a mechanism (rather than citing the section that owns it) is the drift surface — delete it or cite it. This is the third catalogued sibling of type-contract-vs-test-plan drift and test-bullet-vs-test-vehicle drift (PHASE_20: six instances across four sub-phases). **Every invariant and matrix row also names its falsifier** — the observable output that differs when the property breaks. An oracle built from the function under test (or its definition) is not one, and "deterministic" means equal on separately built equal inputs, not on the same term twice; an unnamed falsifier gets filled with a tautology (see `.work/retro/2026-09-22-phase-23-compact-tools_applied.md` F3).
...
32. **Out-of-Module-Tree debt needs a tree.** A multi-sub-phase design ends with an explicit `[CHORE]` sweep sub-phase carrying its own Module Tree, which lists as `(MODIFY — N.M)` every `lib/`/`test/` file named by a ticket or `[CARRY]` the design already knows about. "File a ticket" is a legal disposition only for a ticket raised after the design locked, or one whose fix the sweep's Verification cannot gate; a re-filed HANDOFF row stays Open with its ASKS pointer. Otherwise every deferral accumulates: Phase 20 closed at 16 tickets filed / 1 resolved because 20.7's tree was docs-only, and Phase 25.7 (tree: `ASKS.md` + one steering file) filed 14 entries and closed 0 code tickets.
...
**Cloned helper families get a support-module row.** When a sub-phase builds a module by transcribing a sibling ("shaped after X", "X transcribed per capability") or adds the second or later adapter of one capability, the Module Tree carries a `(NEW|MODIFY — N.M)` row for the shared support module its private helpers land in (`lib/allm/providers/support/` exists), or the design names a `[DEFERRED-DRY]` listing the helpers and the sub-phase whose tree owns the extraction. Phase 25 deferred duplication in four of six batches because no row permitted it (see `.work/retro/2026-09-24-2026-09-24_SST_SUPPORT_applied.md` F1).
...
**A Test Plan or checklist bullet referencing an existing gate, test, or `file:line` carries the command that finds it, run at design time.** `CLAUDE.md` is loaded as truth into every agent including the design agent, so a false sentence there becomes one error per downstream document, each looking independently corroborated. Worked example, both directions in one phase: PHASE_22.6's Test Plan transcribed a "fenced-API denylist" gate that had never existed, and the implementer — reasonably — shipped an uncompilable fenced example believing fences carried coverage, costing a High finding one sub-phase later; PHASE_22.7's checklist wrote "verified clean at HEAD" beside a stale conformance-format claim and cost nothing.
