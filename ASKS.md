...
[MILE] mon 8/31 7pm - Added the Layer A moderation data types, validator, and error enum for Phase 22.1, plus a shared-helper extraction across the embeddings and moderation validators

[IMPL] mon 8/31 8pm - Implement Phase 22.2 — the ALLM.ModerationAdapter behaviour, the Engine :moderation_adapter field, ALLM.Providers.FakeModeration, and the published ALLM.Test.ModerationAdapterConformance suite

[ASKS] mon 8/31 8pm - [BUG] test/allm/embedding_adapter_test.exs:46 fails when the file is run ALONE (`mix test test/allm/embedding_adapter_test.exs` → 1 failure) and passes in any run that requires two or more test files. The test asserts `capture_io(:stderr, fn -> Code.compile_string(source) end) == ""` while the surrounding test body ALSO carries `assert MinimalImpl.max_batch_size() == 16` against a module the compiler cannot see; Elixir defers that undefined-remote-call diagnostic, and whether it lands inside the capture depends on how the parallel compiler batches the run. So the test is not asserting what it claims — a real warning emitted by the compiled source would be indistinguishable from this artifact. Phase 22.2's sibling (test/allm/moderation_adapter_test.exs) avoids it by binding the module from `ExUnit.CaptureIO.with_io/2` + `Code.compile_string/1`'s return value instead of aliasing it, so the assertion is about the compiled source and nothing else. Out of 22.2's Module Tree (CLAUDE.md cross-phase discipline), hence this ticket. DONE WHEN: `mix test test/allm/embedding_adapter_test.exs` exits 0 as a single-file run, and `grep -n 'MinimalImpl\.' test/allm/embedding_adapter_test.exs` returns nothing (the alias-based reference is gone).

[CDRV] mon 8/31 7pm - Code review Phase 22.2 moderation Layer B runtime contract, FakeModeration, and conformance harness

[FIX] mon 8/31 9pm - Fix Phase 22.2 moderation Layer B review findings — route FakeModeration's retry budget through the cursor-key precedence, derive the conformance suite's batch sizes from max_batch_size/0, split the spent-script fallback from the no-script default, append behaviour invariants 9 and 10, define invariant 5 in items rather than list elements, and file the deferred-DRY and CARRY tickets

[ASKS] mon 8/31 9pm - [DEFERRED-DRY] ALLM.Providers.FakeModeration is a 64% verbatim clone of ALLM.Providers.FakeEmbeddings — 131 of 205 normalized code lines match byte-for-byte after module-name substitution. The cloned surface is advance_cursor/2, advance_process_dict_cursor/2, peek_cursor/2, cursor_key_id/2, start_script_cursor/0, cursor_index/1, maybe_capture/2, handle_retry_until_call/6, bump_retry_visits/3, both gate/1 clauses, run_scripted/2, moderate/2's body, script/1, validate_entry!/1 and invalid_entry!/1 — identical modulo the process-dict key atom (:allm_fake_embeddings_cursor vs :allm_fake_moderation_cursor), the error module, the struct in each function head, and the exhaustion arm. agent-spec/IMPLEMENTATION.md:68's two-implementations trigger fired at FakeImages (copy two) and was missed; copy four shipped in 22.2. High-fidelity copying is how a defect at fake_embeddings.ex:391 became a defect at fake_moderation.ex:424 (see the [CARRY] ticket below) — the cost of deferring is that copy four inherits copy three's bugs. Extraction was correctly declined inside 22.2 because the Module Tree scopes only fake_moderation.ex; note that CLAUDE.md's cross-phase discipline carries a NAMED EXCEPTION for extraction-trigger refactors that are private, behaviour-preserving, leave every public name in place and are pinned by the prior phase's tests — which this one would be — so the honest ground for deferral is IMPLEMENTATION.md:68's "the Module Tree wins", not "CLAUDE.md forbids it". Proposed shape: `use ALLM.Providers.Support.ScriptedAdapter, script_key: :moderation_script, error_module: ModerationAdapterError`, with an ALLM.Providers.Support.ScriptCursor covering the cursor half. DONE WHEN: `grep -l 'defp advance_process_dict_cursor\|defp bump_retry_visits\|defp maybe_capture\|defp invalid_entry!' lib/allm/providers/*.ex` comes back EMPTY. Today it returns fake.ex, fake_embeddings.ex, fake_images.ex and fake_moderation.ex, and it scores partial progress honestly as each module is migrated. (The narrower predicate first recorded in steering/2026-08-31_PHASE_22_moderation_RECORDS.md — advance_process_dict_cursor alone — would have scored green with about 40% of the duplication still in place and is superseded.)

[ASKS] mon 8/31 9pm - [DEFERRED-DRY] conformance/test/support/fixtures/scripted_moderation_stub.ex and scripted_embedding_stub.ex share start_script_cursor/0, advance/1, both gate/1 clauses and build_response/3's shape — roughly 40 of 145 lines. Filed for the record that IMPLEMENTATION.md:68's trigger was considered, NOT as work to schedule: the deferral is judged correct and the code review agreed ("No change recommended; the RECORDS entry's reasoning holds"). The largest shared block is gate/1, whose entire purpose is that each stub evaluates its own real gates rather than delegating — sharing it would defeat the fixture — and what remains (start_script_cursor/0 + advance/1, eight lines) does not justify a module coupling three behaviours' conformance fixtures. DONE WHEN: nothing; re-evaluate only if a fourth stub arrives, i.e. when `ls conformance/test/support/fixtures/scripted_*_stub.ex | wc -l` returns 4 or more. Today it returns 3.

[ASKS] mon 8/31 9pm - [CARRY] ALLM.Providers.FakeEmbeddings and ALLM.Providers.FakeImages key their retry-visit counter on the script content hash and ignore adapter_opts[:cursor_key], so two engines with distinct :id values and content-equal scripts get SEPARATE cursor slots but a SHARED retry budget in one process. Engine A burning a {:retry_until_call, 3} budget leaves the counter at 2; engine B's first call reads 3, skips both rate-limited returns and succeeds immediately — a retry test that fails GREEN, which is the exact failure mode the :cursor_key mechanism exists to prevent. lib/allm/providers/fake_embeddings.ex:391 and lib/allm/providers/fake_images.ex:370 both read `key = {:allm_fake_*_retry_visits, :erlang.phash2(script), cursor}` while their own cursor_key_id/2 honours the documented three-source precedence, so each module contradicts its own `## Cursor behaviour` moduledoc section. Phase 22.2's fix pass corrected the identical defect in lib/allm/providers/fake_moderation.ex (bump_retry_visits/3 now keys on cursor_key_id/2, and cursor_key_id/2 covers the :script_cursor pid too) and shipped two regression tests; the two siblings are RELEASED and outside 22.2's Module Tree, hence this ticket per CLAUDE.md's "shipping safer than an already-released sibling" rule. lib/allm/providers/fake.ex uses a different multi-call mechanism and is not implicated. DONE WHEN: `grep -n 'phash2(script), cursor' lib/allm/providers/*.ex` comes back EMPTY — no Fake adapter derives a retry-visit key from the script hash any more. Today it returns fake_embeddings.ex and fake_images.ex. Each fix also needs the FakeModeration regression test ported ("two content-equal-script engines with distinct :id values do not share a retry budget", test/allm/providers/fake_moderation_test.exs).

[MILE] mon 8/31 8pm - Added the moderation adapter behaviour, engine slot, FakeModeration, and a ten-case published conformance harness for Phase 22.2, fixing a shared-retry-budget cursor defect

[IMPL] mon 8/31 8pm - Implement Phase 22.3 — the ALLM.moderate/3 and ALLM.moderation_request/2 Layer C façade, the [:allm, :moderate] telemetry span, and ALLM.Capability.preflight_moderation/2

[CDRV] mon 8/31 8pm - Code review Phase 22.3 Layer C moderation facade (ALLM.moderate/3, Capability preflight, telemetry span)

[FIX] mon 8/31 9pm - Fix Phase 22.3 Layer C moderation review findings: bind the capability gate with a facade wiring test, extract the four-copy capability flag matcher, and correct false telemetry/retry/batching doc claims.

[ASKS] mon 8/31 9pm - [CARRY] ALLM.embed/3's capability pre-flight is wired into the façade but bound by no test — the identical gap Phase 22.3's fix pass closed for ALLM.moderate/3. Deleting `:ok <- ALLM.Capability.preflight_embedding(resolved_model, request)` from do_embed_body/5's `with` in lib/allm.ex leaves the FULL suite green (measured by the 22.3 functional reviewer against all three siblings: neutering preflight_image/2 in generate_image/3 gives 2 failures because ALLM.CapabilityImageTest carries explicit 'wired into the façade' cases; neutering preflight_embedding/2 in embed/3 gives 0; neutering preflight_moderation/2 in moderate/3 gave 0 before the fix). So the one thing a test can prove about a no-op-by-default gate — that it is plugged in — is unproven for embeddings: a model the catalog says cannot embed would round-trip to the provider and fail on the wire instead of failing locally, and nothing in CI would notice. Fix shape: copy test/allm/allm_moderate_test.exs's 'gate 3 — Capability.preflight_moderation/2 is wired into the façade' and 'gate 2 precedes gate 3' pair into test/allm/allm_embed_test.exs, driving ALLM.embed/3 against an engine whose :model is a bare %ALLM.ModelRef{} with capabilities: %{embeddings_enabled: false}. That needs NO Application.put_env — test/support/llm_db.ex's `model(%ModelRef{} = ref), do: ref` identity clause is compiled in :test — so the file stays async: true and the :force_capability_absent override (an async: true foot-gun, see test/allm/capability_vision_test.exs:81) is not needed. Verify by mutation: deleting the with-clause must turn the new test red. RELEASED code outside Phase 22.3's Module Tree, hence this ticket per CLAUDE.md's 'shipping safer than an already-released sibling' rule. DONE WHEN: `grep -l unsupported_capability test/allm/allm_*_test.exs` names one file per capability façade — today it returns only allm_moderate_test.exs, and allm_embed_test.exs is the file missing.

[ASKS] mon 8/31 9pm - [BUG] The :telemetry_metadata call-control opt is advertised in public @doc and stripped by a deny-list, but no code in lib/ ever READS it. Re-measured at the Phase 22.3 fix pass: `grep -rn telemetry_metadata lib/` returns exactly two occurrences of the OPT name — lib/allm.ex:973 (ALLM.embedding_request/2's '## Options' paragraph, which lists it among the call-control opts embed/3 forwards) and lib/allm.ex:1402 (drop_request_opts/1's deny-list on the image path, which strips a key nobody sets). Every other hit is an unrelated local variable of the same name in the ALLM.Retry / ALLM.EmbeddingBatch plumbing (retry.ex:263, embedding_batch.ex:41, and the three telemetry_metadata = %{...} bindings at lib/allm.ex:1488, :1662, :1825), none of which is fed from opts. It appears in no guide, not in README.md, and not in steering/allm_engine_session_streaming_spec_v0_2.md. A user who passes it gets silence: on the image path it is stripped, on the embed path it is forwarded to the adapter as an unknown opt, and the correlation tag never reaches a telemetry handler. Phase 22.3's fix pass removed the advertisement from ALLM.moderation_request/2's @doc (the moderation path did not even strip it — it was forwarded to the adapter), leaving the released embeddings twin standing. Decide one of two directions and apply it repo-wide: either IMPLEMENT it — Map.merge(opts[:telemetry_metadata] || %{}, start_metadata) in do_generate_image/3, do_embed/3 and do_moderate/3, plus the same merge into each Retry.run/3 telemetry_metadata map, with a test per façade asserting a caller-supplied tag reaches both the span's :start metadata and the [:allm, :adapter, :retry] event — or DELETE the name from lib/allm.ex:973 and lib/allm.ex:1402 and re-add it to moderation only if it is ever implemented. DONE WHEN: `grep -rn telemetry_metadata lib/allm.ex` returns only lines that READ or MERGE the key (today: two lines that do neither).

[MILE] mon 8/31 9pm - Added the ALLM.moderate/3 facade, moderate telemetry span, and capability pre-flight for Phase 22.3, binding a gate that had been wired but untested

[CDRV] mon 8/31 11pm - Code review Phase 22.4 OpenAI moderation adapter against the embeddings template and CLAUDE.md invariants

[FIX] tue 9/1 12am - Fix Phase 22.4 OpenAI moderation adapter review findings from .work/reviews, code-reviews, and security-reviews

[ASKS] tue 9/1 12am - [CHORE] Extract the recorder scaffolding shared across scripts/record_*.exs into scripts/support/fixture_recorder.exs (a FixtureRecorder module exposing load_dotenv/1 taking the env-var name, overwritable?/1, write!/2, pending_paths/1, verdict/2, record_probe_body/2; halt_unless_schema_holds/1 stays per-script because its body is the provider-specific diagnostic) and migrate every copy in the same commit per agent-spec/IMPLEMENTATION.md's "Migration on extraction" rule — load_dotenv/0 and overwritable?/1 are each at four byte-identical copies, and overwritable?/1 is the refuse-to-overwrite guard the whole synthesized/recorded provenance discipline rests on, so four independently-maintained copies of a safety check is how one drifts and a live recording gets clobbered; candidate for 22.7, DONE WHEN `grep -l 'defp load_dotenv\|defp overwritable?' scripts/record_*.exs` is empty

[MILE] tue 9/1 12am - Added the OpenAI /v1/moderations adapter with a live wire probe that measured max_batch_size and confirmed text-moderation-latest is shut down

[IMPL] tue 9/1 12am - Implement Phase 22.5 of the moderation design: image input for ALLM.Providers.OpenAI.Moderation (%ImagePart{} items as image_url content blocks, the MIME/20MB pre-flight gate ahead of key resolution, the multimodal one-result cardinality rule, and the live multimodal fixture recording)

[CDRV] tue 9/1 12am - Code review on Phase 22.5 moderation image-input support

[ASKS] tue 9/1 6pm - Review the current status of steering/2026-08-31_PHASE_22_moderation.md, which was mid-build when the container died

[CDRV] tue 9/1 6pm - Code review Phase 22.5 moderation image-input support (re-run after the container outage)

[FIX] tue 9/1 6pm - Fix Phase 22.5 moderation image-input findings from the functional, code and security reviews

[ASKS] tue 9/1 7pm - [DEFERRED-DRY] detail_drop_check/1 + warn_detail_dropped_once/0 now exist in THREE copies: lib/allm/providers/gemini.ex:755-773, lib/allm/providers/anthropic.ex:883-899, and lib/allm/providers/openai/moderation.ex (new in Phase 22.5). Identical modulo the process-dictionary key atom (:allm_gemini_detail_warned / :allm_anthropic_detail_warned / :allm_openai_moderation_detail_warned) and the log string. agent-spec/IMPLEMENTATION.md:68 sets the extraction trigger at TWO implementations and says explicitly that it is semantic not byte-level, so it fired at copy two (Gemini, Phase 16.4) and was missed; copy three shipped in 22.5 and the 22.5 fix pass is filing the record IMPLEMENTATION.md:68 requires when the Module Tree forecloses consolidation. Extraction was correctly declined inside 22.5 because it edits two RELEASED adapters outside the sub-phase's Module Tree. Proposed shape: ALLM.Providers.Support.DetailDrop with drop_check(detail, process_key, provider_label), sitting alongside the existing ImageMime / OpenAIHeaders / GeminiHeaders support modules. NOTE a real behavioural divergence to settle at the same time, not just a refactor: gemini.ex:755 excludes :auto from the warning (:auto is ALLM.ImagePart's DEFAULT, so nothing was really dropped) and openai/moderation.ex now matches it after the 22.5 fix pass, but anthropic.ex:883 does NOT — so Anthropic fires a debug log on every plainly-constructed ImagePart.new/1. Pick one rule for the family when extracting. DONE WHEN: grep -l 'defp warn_detail_dropped_once' lib/allm/providers/*.ex lib/allm/providers/*/*.ex comes back EMPTY - today it returns three files, and it scores partial progress honestly as each adapter migrates.

[ASKS] tue 9/1 7pm - [CARRY] lib/allm/providers/openai.ex:1864 raises MatchError for a file-sourced image whose file cannot be read. The released Chat Completions vision translator does `{:ok, uri} = Image.to_data_uri(img)` as a hard match, and Image.to_data_uri/1 returns {:error, :enoent} for a {:file, path} whose file is missing (lib/allm/image.ex:308-313). Nothing upstream catches it: ImageMime.check_byte_size/1 DELIBERATELY returns :ok when Image.to_binary/1 fails (lib/allm/providers/support/image_mime.ex:126-129, 'the adapter cannot prove the image is OK without bytes'), so MIME and size validation both PASS for a file that is not there. Reproduced directly, not inferred from similarity: ALLM.Providers.OpenAI.to_openai_messages/1 on a message whose content is [ImagePart.new(%Image{source: {:file, "/nonexistent/nope.png"}, mime_type: "image/png"})] raises '** (MatchError) no match of right hand side value: {:error, :enoent}'. The triggering input is mundane - a path valid when the ImagePart was built and deleted or moved before the call, or simply typo'd - and the raise escapes ALLM.generate/3 rather than surfacing as {:error, %AdapterError{}}. Phase 22.5's fix pass fixed the SAME defect in the new lib/allm/providers/openai/moderation.ex by gating byte-resolvability in gate_images/2 (arm {:unresolvable_image, reason} -> :invalid_request with metadata %{image_error: :unresolvable_image, cause: reason}), which is CLAUDE.md's 'shipping safer than an already-released sibling' deviation; this ticket is the record that rule requires in the same pass. Check the Responses-API clause and the Anthropic and Gemini vision translators for the same hard match while fixing - they were not individually reproduced. DONE WHEN: grep -n '{:ok, uri} = Image.to_data_uri' lib/allm/providers/*.ex lib/allm/providers/*/*.ex comes back EMPTY, with a test per fixed site driving a missing file through the public facade and asserting an {:error, _} tuple rather than a raise.

[MILE] wed 9/2 2pm - Added image input to the OpenAI /v1/moderations adapter for Phase 22.5, with the multimodal cardinality rule confirmed live and two invariant-2 crashes gated

[ASKS] wed 9/2 2pm - [RESOLVED] The [CARRY] filed 2026-09-01 for lib/allm/providers/openai.ex:1864 (MatchError on an unreadable image file) is fixed, and its own DONE-WHEN predicate was WRONG TWICE - recorded here because the ticket scored itself green on neither count. (1) It named 'grep {:ok, uri} = Image.to_data_uri' as the predicate, which would have MISSED two of the four affected sites: anthropic.ex:867 uses {:ok, bytes} = Image.to_binary(img) and gemini.ex:745 uses File.read!(path). All four were reproduced before fixing: openai.ex Chat Completions and Responses both MatchError, anthropic.ex MatchError, gemini.ex File.Error. (2) The predicate assumed the fix would DELETE the hard matches. It does not and should not: the fix went into the shared pre-flight gate, so the translators stay total over what the gate admits and STILL raise if called directly on an unresolvable image - which is now asserted on purpose by openai_vision_test.exs's 'the translators are total only over what the gate admits'. Actual fix: ALLM.Providers.Support.ImageMime.check_byte_size/1 stopped folding 'cannot read the bytes' into 'no size objection' and now returns {:error, {:unresolvable_image, reason}}, which covers openai.ex generate/2 + stream/2 and anthropic.ex generate/2 + stream/2 through validate_request/2 (and both OpenAI endpoints, since the gate runs ahead of endpoint dispatch); Gemini keeps its own gate because validate_request/2 accepts only :openai | :anthropic, so check_part_source/1 gained a readability clause returning AdapterError :invalid_request. ALLM.Providers.OpenAI.Moderation's local resolvable?/1 was removed in favour of the shared check. One deliberate widening beyond the crash fix: an undecodable {:base64, _} source is now rejected locally instead of being forwarded for the provider to 400 on. Eight tests added across the four vision test files, each verified by mutation. GENERAL LESSON worth more than the ticket: a DONE-WHEN grep written from ONE reproduced site generalises by syntax rather than by defect, and both of this one's errors came from that - it matched the shape of the one call it had seen instead of the question being asked ('can these bytes fail to resolve?').

[MILE] wed 9/2 2pm - Fixed unreadable image files crashing all four vision translators by making the shared ImageMime gate report unresolvable bytes instead of forgiving them

[ABLD] thu 9/3 6pm - Auto-build the remaining phases (22.6 docs and 22.7 chore sweep) of steering/2026-08-31_PHASE_22_moderation.md through the gate, commit, and retro cycle

[BUILD] thu 9/3 6pm - Build phases 22.6 (spec §39, guide, examples, docs wiring) and 22.7 (chore sweep) from steering/2026-08-31_PHASE_22_moderation.md

[IMPL] thu 9/3 6pm - Implement Phase 22.6 - spec §39, guides/moderation.md, examples 19-20, docs wiring (no release)

[ASKS] thu 9/3 6pm - [CHORE] mix.exs @guides and test/guides_test.exs @guides name DIFFERENT sets, so any guide in the former but not the latter is gated on nothing - it ships to hexdocs with no >2KB check, no banned-token audit, no iex>-block requirement, and no doctest_file/1 execution. Today that is guides/fakes.md, unchecked since Phase 16. This is exactly the hand-maintained-audit-literal fail-open hazard CLAUDE.md describes, and it needs the meta-test that rule prescribes: assert the two literals name the same set, with an explicit @excluded %{name => reason} map so opt-out stays possible and visible. Adding fakes.md also subjects it to doctest_file/1 for the first time and may surface real defects in it - fixing those is in scope. Phase 22.6 registered guides/moderation.md in BOTH literals plus test/guides_doctest_test.exs, so it does not widen the divergence, but it cannot close it: 22.6's Module Tree is docs-only and does not scope test/guides_test.exs's structural rewrite or whatever fakes.md turns out to need. Deadline 22.7, whose Module Tree DOES list test/guides_test.exs. DONE WHEN this comes back empty: diff <(grep -oE 'guides/[a-z_]+\.md' mix.exs | sort -u) <(grep -oE '[a-z_]+\.md' test/guides_test.exs | sed 's|^|guides/|' | sort -u)

[REVW] thu 9/3 6pm - Functional review of Phase 22.6 moderation documentation layer (spec §39, guides/moderation.md, examples 19-20, docs wiring)

[CDRV] thu 9/3 6pm - Code review on current session

[ASRV] thu 9/3 6pm - Architecture and security review on Phase 22.6 moderation docs and examples batch

[FIX] thu 9/3 6pm - Fix Phase 22.6 moderation-docs review findings: uncompilable guide fence, false corruption-signal claim, and the moderation_engine/1 DRY trigger

[MILE] thu 9/3 7pm - Documented content moderation for Phase 22.6 — spec §39, a doctest-protected guide, and two free example scripts — fixing a flagship guide recipe that did not compile

[IMPL] thu 9/3 7pm - Implement Phase 22.7 - the [CHORE] sweep: strike two false CLAUDE.md claims, add the mix.exs/@guides parity meta-test and register guides/fakes.md, port Phase 20.4's error-struct hardening to both image adapters, and correct the false corruption-signal claim in lib/ and the spec

[ASKS] thu 9/3 8pm - [RESOLVED] The [CHORE] filed 2026-09-03 for the mix.exs / test/guides_test.exs @guides divergence is closed by Phase 22.7. Its own DONE-WHEN predicate now comes back empty: diff <(grep -oE 'guides/[a-z_]+\.md' mix.exs | sort -u) <(grep -oE '[a-z_]+\.md' test/guides_test.exs | sed 's|^|guides/|' | sort -u) -> no output, exit 0. guides/fakes.md is registered in all three literals (mix.exs docs[:extras], test/guides_test.exs @guides, test/guides_doctest_test.exs doctest_file/1), and four parity meta-tests now assert the literals against discovered sets in both directions with an explicit @excluded %{} map. What registration surfaced was WORSE than the ticket predicted: the four banned-token audit hits were there as forecast (repo baseline 10 hits / 5 files -> 6 hits / 4 files), but fakes.md also had ZERO iex> blocks, so the >2KB and audit gates were not the only ones it was escaping - the iex> gate had nothing to bind on either, and doctest_file/1 had never executed a line of it. The single block converted from a fence to iex> failed on first execution with KeyError key :content not found - %ALLM.Response{} exposes :output_text, not :content. [CORRECTED thu 9/3 9pm by the 22.7 fix pass, twice: (a) that KeyError was NOT a shipped defect - git show 45e114f:guides/fakes.md shows the pre-22.7 fence stopping at ALLM.Engine.new(...), never calling ALLM.generate/2 and never touching a %ALLM.Response{}, so the wrong field name came from assertion lines authored during the conversion itself; (b) this ticket's own "unchecked since Phase 16" was wrong - guides/fakes.md was created AND registered in mix.exs by 85f45d8 (Phase 21, 2026-05-25), per git log --follow --diff-filter=AR -- guides/fakes.md and git log -S 'guides/fakes.md' -- mix.exs, each returning that one commit. What registration actually caught, and it is enough: a guide mix.exs ships to hexdocs with ZERO iex> blocks and four banned-token hits, gated by nothing. The case for converting Fake-runnable fences survives intact; the shipped-defect framing does not.] All four parity tests verified binding by mutation (drop fakes.md from @guides -> 2 red; unregister its doctest_file/1 -> 1 red; add a phantom name to @excluded -> 1 red).

[ASKS] thu 9/3 8pm - [RESOLVED] The [BUG] filed 2026-07-29 to port Phase 20.4's error-struct hardening to the image adapters is closed by Phase 22.7. Its predicate ("grep -rn 'redact' lib/ hits only the new embeddings module") now hits six provider modules including both image adapters: grep -rln redact_key_material lib/allm/providers/ -> gemini/embeddings.ex, gemini/images.ex, openai/embeddings.ex, voyage/embeddings.ex, openai/moderation.ex, openai/images.ex. Both adapters funnel every non-2xx through a single to_image_adapter_error/4 (verified by reading run_one_attempt/3 -> classify_http_error/4 in each), so redaction is STRUCTURAL rather than status-conditional; each has its own pattern (sk-|rk-|org- for OpenAI, AIza...|ya29.... for Gemini) with a companion test asserting the sibling patterns match nothing in the same fixture, so an inherited-verbatim regex fails loudly. sanitize_cause/1 is applied at every cause assignment on the provider-response path and inspect(cause) no longer reaches :message. [COUNT CORRECTED thu 9/3 9pm: "all three ... in each adapter" was wrong both ways. grep -n 'cause:' lib/allm/providers/openai/images.ex -> SIX (three sanitized at the response funnel; three in fetch_url_bytes/2 still take the raw exception, inert today only because that path sets decode_body: false); grep -n 'cause:' lib/allm/providers/gemini/images.ex -> FOUR, all sanitized.] [PREDICATE AMENDED thu 9/3 9pm: this ticket closed on grep -rln redact_key_material lib/allm/providers/ returning six FILES, which measures name presence, not coverage. The three chat adapters have none - grep -c redact_key_material lib/allm/providers/{openai,gemini,anthropic}.ex -> 0/0/0 - and ALLM.Error.AdapterError has a Jason.Encoder impl (lib/allm/error/adapter_error.ex:170) while Gemini.classify_error/3 assigns :message straight off the untrusted body. Closing against the ticket's own title ("port to the IMAGE adapters") stands; the residual chat gap is refiled below as its own [CARRY] with a coverage predicate.] body_preview is REMOVED outright and not replaced: the ticket proposed swapping it for the body's sorted top-level key list, but the 20.4 template that actually shipped (openai/embeddings.ex malformed_error/4, and the gemini and voyage siblings) passes a bare %{}, so family consistency and the 22.7 design checklist both say drop. grep -rn 'body_preview:' lib/allm/providers/ is empty. Eleven tests added across the two files, six of them verified binding by mutation - notably, swapping Gemini's pattern for the inherited OpenAI one turns three tests red rather than silently redacting nothing.

[REVW] thu 9/3 7pm - Functional review of Phase 22.7 [CHORE] sweep — CLAUDE.md claim strikes, @guides parity meta-test, image-adapter key redaction, corruption-signal correction

[CDRV] thu 9/3 7pm - Code review on current session

[ASRV] thu 9/3 7pm - Architecture and security review on Phase 22.7 chore batch (key-material redaction and body_preview removal in image adapters)

[FIX] thu 9/3 9pm - Fix Phase 22.7 [CHORE]-sweep review findings across four review files: the governed-document falsehoods in CLAUDE.md, the over-claiming redactor provenance comments in both image adapters, the wrong :cause counts, and the self-scoring predicate that matches its own prose

[ASKS] thu 9/3 9pm - [CARRY] Provider-authored strings read off an untrusted response body land on %ImageAdapterError{}/%EmbeddingAdapterError{}/%ModerationAdapterError{} :metadata UNREDACTED, while redact_key_material/1 is applied only to :message. Measured thu 9/3 through the @doc false seams: to_image_adapter_error(400, %{"error" => %{"message" => "nope", "code" => "sk-proj-FAKEKEY000111222333444555", "type" => "invalid_request_error"}}, [], []) yields metadata %{status: 400, openai_code: "sk-proj-FAKEKEY000111222333444555", ...} and the planted key survives BOTH inspect/1 and Jason.encode!/1; the Gemini mirror leaks the same way through error["status"] -> metadata.google_status. These structs derive Jason.Encoder and their own moduledocs say they are routinely logged and persisted, so a proxy or gateway that can populate error.message can equally populate error.code and route straight past the redactor. Family-wide and pre-existing, NOT introduced by 22.7 (openai/images.ex, openai/embeddings.ex, openai/moderation.ex, gemini/images.ex, gemini/embeddings.ex; voyage/embeddings.ex is already clean because it passes only %{status: status}). 22.7 correctly mirrored the Phase 20.4 template rather than diverging, and its comment blocks now carry an explicit SCOPE paragraph saying so instead of reading as an exhaustive audit. Fix: route every provider-authored string value through the module's own redact_key_material/1 before it reaches build_metadata/2 - per-provider, because inheriting a sibling's regex is a silent no-op. DONE WHEN: for each of the five modules, planting a key-shaped token in the body field that feeds :metadata and driving the module's error funnel leaves the token absent from both inspect/1 and Jason.encode!/1 - add one test per module in the existing describe "redaction" blocks, and this ticket closes on those five tests existing and binding by mutation, NOT on a grep for the helper's name (see the [BUG] above for why a name-presence predicate scored a false green).

[ASKS] thu 9/3 9pm - [CHORE] [DEFERRED-DRY] sanitize_cause/1 is now byte-identical in SIX provider modules and provider_message/2 in TWO, with no shared home. grep -l 'defp sanitize_cause' lib/allm/providers/*.ex lib/allm/providers/*/*.ex -> gemini/embeddings.ex, gemini/images.ex, openai/embeddings.ex, openai/images.ex, openai/moderation.ex, voyage/embeddings.ex. grep -l 'defp provider_message' lib/allm/providers/*.ex lib/allm/providers/*/*.ex -> gemini/embeddings.ex, gemini/images.ex. Both sanitize_cause/1 clauses are literally provider-free (defp sanitize_cause(%{__struct__: Jason.DecodeError} = cause), do: %{cause | data: ""}; defp sanitize_cause(cause), do: cause), so there is nothing to diverge on - six hand-maintained copies of a security-shaped sanitiser is six places to update when Jason.DecodeError grows a field or a second cause type needs blanking, and five will be missed. agent-spec/IMPLEMENTATION.md:68 puts the trigger at TWO. lib/allm/providers/support/ already hosts transport.ex, openai_headers.ex, gemini_headers.ex and image_mime.ex, so the destination is not in doubt - propose ALLM.Providers.Support.ErrorHygiene.sanitize_cause/1. EXPLICITLY NOT IN SCOPE: consolidating redact_key_material/1's REGEX. Its body is now identical across the three OpenAI modules modulo the fallback string, but CLAUDE.md is explicit that inheriting a sibling's pattern across providers is a silent no-op and the companion tests enforce per-provider divergence; a shared extraction must take the pattern as an argument. DONE WHEN: grep -l 'defp sanitize_cause' lib/allm/providers/*.ex lib/allm/providers/*/*.ex is empty (today: six files).

[ASKS] thu 9/3 9pm - [CARRY] The three released CHAT adapters put raw provider-authored text into a Jason.Encoder-derived error struct with no redaction, while all six capability adapters now redact. grep -c redact_key_material lib/allm/providers/{openai,gemini,anthropic}.ex -> 0, 0, 0. ALLM.Error.AdapterError derives Jason.Encoder via defimpl at lib/allm/error/adapter_error.ex:170, and Gemini.classify_error/3 assigns message: Map.get(error, "message", ...) straight off the response body - the same untrusted channel gemini/images.ex now redacts on top of. Chat is the library's highest-traffic path. Per CLAUDE.md, shipping safer than an already-released sibling is a correctly-scoped deviation AND evidence of a live weakness in that sibling, so this is the [CARRY] Phase 22.7 owed and did not file. Note test/fixtures/openai/synthesized/auth_failed.json still carries the un-lengthened sk-bad placeholder, which no 6+-character pattern can match, so adding redaction without lengthening it would bind nothing - lengthen it the way 22.7 lengthened the image fixture. DONE WHEN: grep -L redact_key_material lib/allm/providers/openai.ex lib/allm/providers/gemini.ex lib/allm/providers/anthropic.ex is empty (today: all three), AND each adapter has a test planting a provider-specific key-shaped token in the 401 body and asserting it is absent from inspect/1 of the resulting %AdapterError{}.

[ASKS] thu 9/3 9pm - [BUG] Both image adapters' error funnels raise BadMapError on a string-valued "error" envelope, on input the caller does not control. Probed live thu 9/3 at the checkpoint: ALLM.Providers.OpenAI.Images.to_image_adapter_error(500, %{"error" => "boom"}, [], []) and the Gemini mirror both raise ** (BadMapError) expected a map, got: "boom". Cause: each does error = Map.get(body, "error", %{}) then Map.get(error, "code") / Map.get(error, "status") without checking the extracted value is a map. Pre-existing - NOT introduced by Phase 22.7 - but 22.7 hardened the ADJACENT case in the same functions and both new comment blocks reason explicitly about a provider or proxy answering {"error": {"message": 123}}; a proxy answering {"error": "Bad gateway"} is at least as likely, and is not hypothetical in this codebase's own provider set, since Voyage's real envelope is {"detail": "<string>"}. Same class as the OpenAI.Embeddings.embed/2 divergence CLAUDE.md already documents: a public @doc false extension point that raises instead of returning its declared ImageAdapterError.t(). One-line fix at each site: error = case Map.get(body, "error") do e when is_map(e) -> e; _ -> %{} end. DONE WHEN: to_image_adapter_error(500, %{"error" => "boom"}, [], []) returns a %ImageAdapterError{} for BOTH adapters, pinned by one test each.

[ASKS] thu 9/3 9pm - [BUG] sanitize_cause/1 blanks Jason.DecodeError's :data but leaves :position pointing into a now-zero-length binary, producing a struct whose own message/1 impl raises. Six sites: openai/images.ex, gemini/images.ex (both added Phase 22.7), openai/embeddings.ex, gemini/embeddings.ex, voyage/embeddings.ex, openai/moderation.ex. Jason.DecodeError.message/1 (deps/jason/lib/decoder.ex) has three clauses; with token: nil and position > 0 the byte_size(data) clause stops matching once data is "" and the fallback calls :binary.at("", position). MEASURED thu 9/3, and the impact is MILDER than the arch-and-security review stated - re-measure before you write the fix note: Jason.DecodeError.message/1(%Jason.DecodeError{position: 1, token: nil, data: ""}) raises ArgumentError, but Exception.message/1 does NOT propagate it (Elixir wraps the impl and returns the diagnostic string "got ArgumentError with message \"argument error\" while retrieving Exception.message/1 for %Jason.DecodeError{...}"), and Exception.format/3 does not raise either. So an app calling Exception.message/1 or Exception.format/3 gets a degraded, noisy diagnostic rather than a crash; only a direct call to the impl module raises. That makes this a robustness/observability defect, not a crash-in-the-error-handler defect. Fix: make the blanked value internally consistent - %{cause | data: "", position: 0, token: nil} - after which Exception.message/1 returns "unexpected end of input at position 0" (verified). Apply at all six sites in one commit, ideally as part of the ErrorHygiene extraction ticketed above. DONE WHEN: for each of the six modules, Exception.message/1 on a sanitized Jason.DecodeError cause returns a plain "unexpected end of input" string with no "got ArgumentError" prefix, pinned by one test.

[MILE] thu 9/3 7pm - Closed Phase 22's out-of-tree debt in the 22.7 chore sweep — image-adapter key redaction, guide-registration parity gates, and four false claims struck from CLAUDE.md
