...
- **Serializability tests** — every Layer A struct round-trips through both `:erlang.term_to_binary/1` and `ALLM.Serializer.to_json!/1 |> from_json/1`. Required for v0.3 release per agent-spec/DESIGN.md §6.
...
**Coverage threshold:** 80% global per `mix.exs:19`; ≥90% on new code per agent-spec/DESIGN.md. The `Validate.image_request/1` matrix (≥30 assertions across happy + error paths) takes the validator above 90% on its own; the four struct constructors are simple struct + getter shape and reach 100% trivially.
