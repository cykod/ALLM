...
### 2.3 Tag locally, then push from a GitHub-connected workstation (after publish succeeds)

In the release container (hex.pm-connected, GitHub-disconnected):

```bash
git tag -a v0.3.0 -m "v0.3.0 — Multimodal foundation"
git show v0.3.0           # eyeball the local tag
```

Then on a workstation with GitHub access (sync the container's commit + tag in via your provisioning mechanism — rsync, mounted volume, etc.):

```bash
git push origin main v0.3.0
```

The annotated tag is what `mix.exs:76 source_ref: "v#{@version}"` resolves to in hexdocs `[source]` links — until the tag lands on GitHub, hexdocs `[source]` links resolve to a 404 (expected; gone within seconds of pushing).

The publish-then-tag-then-push ordering is deliberate. `mix hex.publish` is the only irreversible step (1-hour revert window after the first 24 hours); once it succeeds the release version is locked on Hex regardless of local git state. The local commit + tag are cheap and recoverable, so they happen next. The push is a separate manual step on a different machine because (a) the release container has no GitHub credentials, and (b) even if it did, a maintainer who spots something wrong in the local tag can fix it without coordinating with anyone who already pulled from origin, and (c) a transient push failure won't strand the release in a published-but-not-pushed state.
...
4. **Up-to-date check (offline)** — print a reminder that the maintainer is responsible for ensuring `main` carries every change intended for this release. The script does NOT `git fetch origin` because the release container has hex.pm credentials but no GitHub auth; network calls against origin would fail. The maintainer pulls pending main changes on a workstation with GitHub access and syncs the checkout into this container before invoking.
5. **Tag-doesn't-exist check (local-only)** — `git rev-parse v<X.Y.Z>` must fail. The script does NOT consult origin (`git ls-remote` requires GitHub auth that this container lacks). If a co-maintainer pushed `v<X.Y.Z>` from elsewhere without syncing here, `mix hex.publish` itself will reject the duplicate version (Hex enforces uniqueness server-side) — that is the backstop.
...
12. **Phase B (`--finalize`) — commit + tag locally (NO push, NO publish).** A second invocation `mix run scripts/release.exs --finalize` reads `mix.exs:@version` (set by Phase A), verifies the CHANGELOG entry is still present, and:
    ...
    - `git commit -m "Release v<X.Y.Z>"` — **but** if `git diff --cached --quiet` reports no staged changes (i.e., the bump and CHANGELOG were already committed in a prior commit, e.g., as part of regular development before the release), Phase B SKIPS the commit and tags the current HEAD directly. The release-commit invariant is "the tag points at a commit that contains the @version bump and the matching CHANGELOG entry"; that invariant is satisfied either by a fresh commit OR by HEAD already carrying both.
...
| Behind main (out-of-band) | Not detectable from this container — origin is unreachable. Maintainer's responsibility to sync before invoking. |
...
- Does NOT contact `origin` at all — no `git fetch`, no `git ls-remote`, no `git push`. The release container has hex.pm credentials but no GitHub auth (no SSH key, no token), so any network call against origin would fail. Step 4 (up-to-date check) is offline and devolves to a maintainer reminder; step 5 (tag check) is local-only with Hex's server-side version-uniqueness as the backstop; on success the script commits + tags locally and prints the exact `git push origin main v<X.Y.Z>` command for the maintainer to run from a workstation with GitHub access. Rationale: this network-isolation pattern ALSO removes the published-but-not-pushed failure mode that an in-script push would create on transient origin failures, and keeps `mix hex.publish` (the only irreversible step) clearly bounded.
