...
6. **CHANGELOG check** — `CHANGELOG.md` must contain a heading line matching `## v<X.Y.Z>` OR `## [Unreleased]` (script auto-rewrites `[Unreleased]` to `v<X.Y.Z>` after gates pass). Reject otherwise — forces the maintainer to write release notes BEFORE running the script.
...
9. **Rewrite CHANGELOG `[Unreleased]` → `v<X.Y.Z>`** if applicable. Show diff.
...
| CHANGELOG missing entry | Abort; print expected heading format |
...
| User answers `n` at confirmation #1 | Roll back the `mix.exs` and `CHANGELOG.md` edits via `git checkout -- mix.exs CHANGELOG.md`. Working tree returns to pre-script state. |
...
- Does NOT auto-edit CHANGELOG content (only renames `[Unreleased]` → `v<X.Y.Z>`). The maintainer writes release notes by hand before running the script.
...
No `lib/` changes. No `test/` changes (the existing `test/allm/release_polish_test.exs` per Phase 17.3 already smoke-tests `mix.exs @version`, CHANGELOG headers, and `examples/_helpers.exs` shape).
