...
| `detail` on `image_url` | not sent | **INFERRED, and confirmed unresolvable 2026-09-01** — Decision #8. 22.5's paired arm posted the identical multimodal body with `detail: "low"` and got **200 with `category_scores` identical to the no-`detail` arm**. Neither observation promotes the row: this endpoint 200s unknown fields, so acceptance is not evidence, and scores are model output, so a difference would not have been evidence either. The drop stands on OpenAI's documented request shape, which carries no `detail` key |
...
| `gate_images/2` `@doc false` | `(request, opts)` → `:ok \| {:error, ModerationAdapterError.t()}`; per-item image gate. **Named `gate_images/2`, not `reject_oversized_images/1`** — both corrections made at implementation time (22.5): arity 2 because every error this adapter surfaces carries `opts[:request_id]` via `build_metadata/2`, exactly as its sibling gates `gate_empty_input/2` and `gate_batch_size/2` do; and *oversized* named one of the five shapes it rejects, where the `gate_*` prefix matches those siblings | 22.5 |
...
- `part_to_block/1 on a URL-sourced ImagePart emits {"type":"image_url","image_url":{"url":<url>}}` — the URL fast-path, never `to_data_uri/1` (which returns `{:error, :remote_source}` for `{:url, _}`, `lib/allm/image.ex:297`; the chat translator's equivalent fast-path is `lib/allm/providers/openai.ex:1853-1863`)
- `part_to_block/1 on a binary-sourced ImagePart emits a data: URI`
- **`part_to_block/1 emits NO "detail" key`** — Decision #8. This test, not the wire, is what holds the decision: the recorder's `detail` arm cannot settle the field's disposition at an endpoint that 200s unknown fields (22.4.6), so the in-code assertion is the only binding one
...
- ~~`moderate/3 with a mixed list returns exactly one result`~~ and ~~`the :start telemetry metadata carries multimodal: true for a mixed list`~~ — **both already shipped in 22.3** against `FakeModeration` (`:282` and `:690` at the 22.5 commit's parent), so 22.5 writes neither. The gap they leave is that on the `FakeModeration` path ALLM is agreeing with itself: the single result is synthesized by ALLM, not returned by a provider. 22.5's actual modification is one test driving the same rule through the **real** `ALLM.Providers.OpenAI.Moderation` against the live `recorded/multimodal_text_image.json`, which additionally pins that the façade hands the adapter an `:input` it can translate — before 22.5 the `%ImagePart{}` reached `Jason.encode!/1` as a bare struct.
...
- [ ] `gate_images/2` (**not `reject_oversized_images/1`** — name and arity both corrected at implementation time; see the seam table row) calling `ALLM.Providers.Support.ImageMime.validate/2` per part with `ImageMime.accept_mimes(:openai)`; convert **all** of its error shapes into `%ModerationAdapterError{reason: :invalid_request}` with the detail on `:metadata` (Decision #7 — **do not** widen the error union): `{:error, {:unsupported_image_format, mime}}`, `{:error, {:image_too_large, bytes}}`, and `{:error, :missing_mime_type}` (`lib/allm/providers/support/image_mime.ex:105-107`). The third is reachable via `ALLM.Image.from_file/1` on a path whose extension is not in `@ext_to_mime` (`lib/allm/image.ex:98-101` leaves `:mime_type` `nil`), or a hand-built `%ALLM.Image{}`. **It is NOT reached via `ALLM.Image.from_url/1`** — corrected at implementation time (22.5): a `{:url, _}` source takes `ImageMime.validate/2`'s *first* clause (`image_mime.ex:94-103`), which explicitly accepts a `nil` mime because URL sources defer to the provider.
- [ ] **Two further failure shapes the gate must own, added at the 22.5 fix pass** — both found by running the code, both reproduced through `ALLM.moderate/3` or a direct `moderate/2` call. `ImageMime.check_byte_size/1` returns `:ok` when the bytes cannot be resolved at all (`image_mime.ex:126-129`), so a `{:file, path}` whose file is missing passes MIME **and** size validation and then raises `MatchError` at `part_to_block/1`'s `{:ok, uri} = Image.to_data_uri(img)`; and `part_to_block/1` has no catch-all, so an item that is neither a binary nor an `%ImagePart{}` in a **multimodal** request raises `FunctionClauseError`. Both escape `moderate/2` as raised exceptions, which is ALLM's own bundled adapter violating invariant 2 — the exact consequence this checklist already cites for the unmatched-clause case. Gate them as `{:unresolvable_image, reason}` and `{:untranslatable_item, item}`. The all-strings path keeps forwarding off-shape items to the provider verbatim; only the multimodal path rejects them, because only it must translate them
...
- [ ] Extend the recorder with the multimodal arm and a `detail`-bearing **paired companion** (`detail` inside `image_url`). **Superseded in-line:** this bullet said "negative control", and 22.4.6's later forward-binding block supersedes it — 22.4's control came back POSITIVE, so no arm at this endpoint can expect a 400 and a control written to expect one would be a test against a rejection that never comes. The companion asserts only the 200 and the single result, and reports the response-observable comparison (`category_scores` with vs without `detail`) as a print, because scores are model output and a strict equality assertion would flake
- [ ] Update the wire-field map's image rows. **Only the multimodal-request row can move**: it is promoted to **confirmed** by the live arm (two content blocks in, exactly one `results` entry back). The `detail` row **stays inferred** and 22.4.6 already establishes that it cannot move at this endpoint — record the observation, not a promotion
...
**Success criterion:** the multimodal recorder arm exits 0 and writes `recorded/multimodal_text_image.json` containing exactly one `results` entry; the `detail` companion arm records what the wire can actually show (a status plus a score comparison) and the wire-field-map row is updated to state the field's disposition is **not resolvable at this endpoint**, per 22.4.6.
