...
- `ALLM.Error.SessionError` reason atoms — added in 8.2 as a scoped Phase-1 vocabulary extension (per `AGENT_DESIGN_SPEC.md` §3 rule 5: design must explicitly extend the prior phase's enum). The four reason atoms are listed in §Error Contract below.
...
This phase adds **four new reason atoms** (`:session_in_error_state`, `:invalid_status_for_operation`, `:no_pending_tool_call`, `:unknown_tool_call_id`) to the project's closed reason-atom set. Per `AGENT_DESIGN_SPEC.md` §3 rule 5, this is a scoped extension of Phase 1's `@type reason :: ...` enum on `ALLM.Error.*`, declared explicitly here. `lib/allm/error/session_error.ex` ships with the four atoms in its `@type reason :: ...`; no other reason atoms are added.
...
Test files mirror source files 1:1 per `AGENT_IMPLEMENTATION_SPEC.md` §Test file organization.
...
property "Session.start/3 ≡ stream_start/3 |> StreamReducer.finalize/1" do
  check all(script <- multi_turn_script_generator()) do
    ...
    input = [ALLM.user("hi")]
    # In-process isolation per AGENT_IMPLEMENTATION_SPEC.md §Property tests.
    {:ok, s1, r1} =
...
**Stream-equivalence relaxation budget** (per `AGENT_DESIGN_SPEC.md` §6 "Stream-equivalence relaxation budget"):
...
**Property-test process isolation** is mandatory — per `AGENT_IMPLEMENTATION_SPEC.md` "Fake-per-process cursor isolation in equivalence properties," wrap each call in `Task.async/Task.await` so the Fake's process-dictionary cursor doesn't shared-cursor between the streaming and non-streaming arms.
...
- [ ] Add `test/allm/session_equivalence_test.exs` with the property test and the `multi_turn_script_generator/0` helper. Use `Task.async`/`Task.await` for cursor isolation (cite `AGENT_IMPLEMENTATION_SPEC.md` rule).
...
- **Status-transition matrix**: the 23-cell table in §Overview is encoded as 23 unit tests in `test/allm/session_status_transition_test.exs`. Each cell has its own `test "transition: <from> --<op>--> <to | error>" do ... end`. Per `AGENT_DESIGN_SPEC.md` §6 "Cross-option × cross-path test matrix" rule, this matrix is exhaustive over the closed status union × the public Phase-8 operation set.
...
- **Doctests**: every new public function has at least one doctest using `ALLM.Providers.Fake`. Per `AGENT_IMPLEMENTATION_SPEC.md` §Per-Phase Loop step 7, a second doctest is added when the function has a meaningfully different branch (e.g., `continue/3` with `nil` message vs. with `%Message{}`).
