...
- **`Stream.resource/3` (NEVER `Stream.unfold/2`)** — per `agent-spec/DESIGN.md` § Elixir-specific. `resource/3` has explicit `after_fun` cleanup; `unfold/2` does not.
...
- [ ] Reviewed via `/review` per `agent-spec/REVIEW.md` AND the additional sub-phase 10.5 validation step (Decision #13): the reviewer runs `OPENAI_API_KEY=… mix run examples/openai/run_all.exs` and records the output in the review artifact. The review is BLOCKING on `run_all.exs` exit-status 0.
