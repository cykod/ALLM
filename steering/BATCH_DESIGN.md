...
> **Layers touched:** A (four new structs) + B (one new behaviour + one new Fake + two new real providers) + C (four new facade functions). Three layers — split into sub-phases for incremental landing per the AGENT_DESIGN_SPEC "one layer per phase" rule when this gets phased.
