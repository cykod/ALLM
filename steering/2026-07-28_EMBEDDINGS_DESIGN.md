...
10. **`:batch_too_large` exists even though the façade never triggers it.** Direct adapter calls (Layer B composability) bypass chunking. Per agent-spec/DESIGN.md rule 13, its current-phase use sites are each adapter's `embed/2` pre-flight guard (20.4–20.6 checklists) and conformance case 3. *Docs target: `ALLM.Error.EmbeddingAdapterError` reason table.*
...
| Truncate | `autoTruncate`, **per sub-request**, nested inside that sub-request's `embedContentConfig` object — NOT a top-level sibling of `requests`. Omitted entirely when `truncate: true` (the provider default); emitted as `false` otherwise. **✅ RESOLVED LIVE during 20.5 — the inferred placement was correct.** See the four-way probe result in 20.5.4; the nested form is accepted and both flat forms are 400s. The `:truncate` field is therefore a real knob on Gemini, unlike OpenAI, and the `to_batch_body/2` test bullet stands as written. |
...
| Usage location | `usageMetadata.promptTokenCount` — **but see the amendment below: absent in practice** |
...
*(Amended during 20.5 — two rows of the table above were wrong about the wire, both discovered by recording rather than by reading.)*

1. **`usageMetadata` is not returned at all.** A live `batchEmbedContents` 200 carries exactly `{"embeddings": [{"values": [...]}, ...]}` — top-level keys `["embeddings"]`, entry keys `["values"]`, nothing else (verified 2026-07-29 against `gemini-embedding-001` on `v1beta`; the single-item `:embedContent` sibling likewise returns only `["embedding"]`). The mapping row stays as specified because the decoder reads the field defensively and will populate automatically if Google starts reporting it, but **`response.usage` is an all-`nil` `%ALLM.Usage{}` on this provider today** and the adapter moduledoc says so. Callers doing per-request cost accounting cannot get it from Gemini. This is the same defect class as 20.4's `type`-vs-`code` finding: the design's claims about *provider* wire behaviour are the ones that have been wrong, and only a recorded response falsifies them.
2. **There is no correlation-id response header**, so the `x-request-id` fallback 20.4 made binding has nothing to read here. Full live header list: `alt-svc`, `content-type`, `date`, `server`, `server-timing`, `transfer-encoding`, `vary`, `x-content-type-options`, `x-frame-options`, `x-xss-protection`. `response.request_id` is `opts[:request_id]` or `nil`. **Binding on 20.6:** check the provider's actual response headers before implementing the fallback; do not port the OpenAI branch on faith.
...
- `embed/2 with a non-string :input ELEMENT returns :invalid_request instead of raising` — added in the 20.5 fix step; the container-shape gate carried forward from 20.4 does not cover elements, and Gemini's body builder reaches inside them. **Binding on 20.6.**
- `translate_reason/1 maps every ALLM.Error.AdapterError.legal_reasons/0 member into ALLM.Error.EmbeddingAdapterError.legal_reasons/0` — added in the 20.5 fix step, and the reason the seam is public at all. Pair it with a `refute` that the wider enum is a subset of the narrower one, or the assertion goes vacuous if the two ever converge.
...
`test/allm/providers/gemini/embeddings_wire_test.exs` (NEW) — five fixtures, same structure as 20.4. *(Four at implementation time; the 20.5 fix step added `recorded/error_400_unknown_field.json`, the placement probe's control-arm 400 envelope, and repointed the provider-envelope assertions onto it.)*

#### 20.5.2 Implementation Checklist

- [x] `lib/allm/providers/gemini/embeddings.ex` — `@behaviour ALLM.EmbeddingAdapter`; `@base_url`; `embed/2`, `max_batch_size/0` → `100`, `prepare_request/2`
- [x] `@doc false` + `@spec` seams: `to_batch_body/2`, `to_gemini_task_type/1`, `decode_response/4`, `to_embedding_adapter_error/4`, `maybe_normalize/2`, `endpoint_url/2`
- [x] **`adapter_opts[:embedding_script]` short-circuit in `lib/`**, mirroring 20.4 (`lib/allm/providers/gemini/images.ex:135-137` is the Gemini-side precedent)
- [x] Per-sub-request `model` stamping with `models/` prefix idempotency
- [x] **Nil-model policy.** Both the URL (`{base}/models/<model>:batchEmbedContents`) and every sub-request's required `model` field derive from `request.model`, which is `nil` until the façade stamps it (dispatch step 6). A direct Layer-B call with `model: nil` therefore CANNOT build a legal request. Return `{:error, %EmbeddingAdapterError{reason: :invalid_request, metadata: %{field: :model}}}` before any HTTP I/O rather than substituting a hardcoded default — deliberately diverging from `lib/allm/providers/gemini/images.ex:510`'s `model || "gemini-3.1-flash-image-preview"` fallback, because a silently-wrong embedding model produces vectors in the wrong space, which is unrecoverable once written to pgvector, whereas a wrong image model is merely a wrong picture. Invariant 10 holds: `embed/2` and `prepare_request/2` reject nil identically.
- [x] `maybe_normalize/2` — normalize iff `dimensions != nil and dimensions != 3072`; delegate to `ALLM.Embedding.normalize/1`
- [x] `to_embedding_adapter_error/4` delegates to `ALLM.Providers.Gemini.classify_error/3` and translates the reason
- [x] Key via `Keys.fetch!(:gemini, opts)` (Decision #11); headers via `GeminiHeaders.headers/1`
- [x] `scripts/record_gemini_embeddings_fixtures.exs` — **includes the `truncate: false` live probe resolving the `autoTruncate` placement question flagged in the wire-field map, before `to_batch_body/2` is finalized**. Per the amended probe contract above, the probe must (i) carry an invented-field control arm, (ii) **assert its expected per-arm statuses and `System.halt(1)` on mismatch** rather than printing, (iii) **write the control arm's 400 body to `recorded/`** rather than discarding it, and (iv) run *behind* the `overwritable?/1` guard so a fully-recorded tree costs zero live calls.
- [x] `test/support/gemini_fixtures.ex` — embeddings loader
...
**Success criterion:** conformance passes; a `dimensions: 768` request yields vectors whose magnitude is within `1.0e-9` of `1.0`; a `dimensions: nil` request yields vectors equal to the fixture values. **All three met** — see 20.5.4.

#### 20.5.4 Implementation Notes

Gates as run: `mix format --check-formatted`, `mix compile --warnings-as-errors`, `mix test` (**2922 tests / 357 doctests / 31 properties, 0 failures**, up from 2810/353/31), `mix credo --strict` (0 issues), `mix dialyzer` (0 errors), `cd conformance && mix test` (90 tests, 0 failures), `mix run scripts/audit_user_docs.exs lib/allm/providers/gemini/embeddings.ex` (0 hits). Coverage: **95.49%** on the new module, **93.26%** global (up from 93.19%).

**★ THE `autoTruncate` PLACEMENT QUESTION IS RESOLVED — the design's inferred nesting was correct.** Resolved by a live four-way probe, not by re-reading the docs, and the probe ships inside `scripts/record_gemini_embeddings_fixtures.exs` so it re-runs on every recording pass. Against `gemini-embedding-001` on `v1beta`, 2026-07-29:

| Arm | Result |
|-----|--------|
| `embedContentConfig: {autoTruncate: false}` on the sub-request | **200** |
| `autoTruncate: false` as a sub-request sibling | 400 `Unknown name "autoTruncate" at 'requests[0]'` |
| `autoTruncate: false` as a top-level sibling of `requests` | 400 `Unknown name "autoTruncate"` |
| **CONTROL:** `totallyNotAField: {}` on the sub-request | 400 `Unknown name "totallyNotAField" at 'requests[0]'` |

The control arm is what makes this conclusive rather than suggestive. A single accepting call proves only that the request was accepted; it becomes evidence of *schema membership* only once the API is shown to reject unknown fields, which the fourth arm establishes. A probe without a control would have been the same shape of reasoning that produced the two wrong wire-field rows amended above.

**Generalizable, and BINDING on 20.6 — the pattern is three parts, and 20.5 shipped one and a half of them before the fix step.** When a design flags a field name, placement, or envelope shape as inferred-not-confirmed, the probe that resolves it must satisfy all three:

1. **Negative control.** Every "the provider accepts X" arm is paired with an invented-field arm in the same run. ✅ shipped in 20.5.
2. **Assert, don't narrate — the probe HALTS the recording pass when its expected shape breaks.** ❌ 20.5 shipped `probe_status/1` formatting the status into a padded string with `run/0` proceeding to record regardless. A probe that prints its verdict into a scrollback is documentation with a network bill: the `Req.Test`-stubbed wire tests assert what the *adapter emits*, so if Google moved `autoTruncate` the suite would stay green forever and the recorder — the only thing in the repo that can observe the change — would print `400` and record happily. *(Fixed in the 20.5 fix step: each arm carries an `:expect` status in `probe_arms/0`; any mismatch prints the full want/got table to stderr and `System.halt(1)`s before a single fixture is written.)*
3. **Record the body, not the status.** ❌ 20.5's probe kept `response.status` and discarded `response.body` on all four arms — including three genuine Google 400 envelopes, which the phase then **hand-wrote** into `synthesized/error_400.json` under a `_comment` claiming *"Envelope shape verified against a live 400 on 2026-07-29"*: a provider-wire claim with no artifact behind it, i.e. exactly the epistemic failure this section condemns, and exactly the shape of the `usageMetadata` row that was wrong. A status code is the cheapest field in a response and almost never the thing under test. *(Fixed in the 20.5 fix step: the CONTROL arm's body is written to `recorded/error_400_unknown_field.json` under the same refuse-to-overwrite guard as every other recorded fixture, added to the wire test's `@recorded` provenance list, and the `:invalid_request` / `google_status` / `details[]` assertions were repointed at it. The synthesized 400 keeps only the redaction assertion — its `AIzaSy…` string is planted by hand and therefore cannot come from a recording — and its `_comment` no longer claims a verification it cannot show. Recording it revealed the hand-written envelope had invented the wrong `details[]` shape: the real one carries `google.rpc.BadRequest` + `fieldViolations`, not `google.rpc.ErrorInfo` + `reason: API_KEY_INVALID`.)*

A fourth obligation, learned the same way: **the probe must run behind the recorder's overwrite guard, not ahead of it.** 20.5's fired at `run/0`'s top, before `overwritable?/1`, so the header's *"a second run against an already-recorded tree is a no-op"* was false — it was four paid live calls. `run/0` now checks every target path first and returns without issuing a single request when they are all recorded.

**Live vs synthesized fixtures, stated plainly:**

* **Live-recorded, committed — three** *(two at implementation time, a third added by the fix step)*. `recorded/batch_embed_contents.json` (3 × 768, `taskType: RETRIEVAL_DOCUMENT`) and `recorded/reduced_dimensions.json` (1 × 256), both recorded 2026-07-29 against `gemini-embedding-001` via `scripts/record_gemini_embeddings_fixtures.exs`; plus `recorded/error_400_unknown_field.json`, the placement probe's CONTROL-arm 400 envelope, which the probe was already paying for and discarding. All three carry **no `_comment` marker**. The recorder's second run is a genuine no-op: it checks every target path before issuing any request and makes zero live calls.
* **Synthesized, marked, committed — two.** `synthesized/error_400.json` and `synthesized/error_429.json`, each carrying a `_comment: "Synthesized (Phase 20.5)…"` marker. Both are **hand-written in full**. *(Amended during the 20.5 fix step: the original text and the fixture's own `_comment` both claimed the 400's envelope was "copied from a live rejection" / "verified against a live 400 on 2026-07-29". Nothing in the tree could check that, and recording the real thing showed it was wrong in a load-bearing way — the genuine envelope's `details[]` carries `google.rpc.BadRequest` with `fieldViolations`, where the hand-written one invented `google.rpc.ErrorInfo` with `reason: API_KEY_INVALID`. All provider-envelope claims — status→reason, `google_status` forwarding, `details[]` shape — are now asserted against the recorded fixture.)* The synthesized 400 survives for one job only: its message embeds a key-shaped `AIzaSy…` string so the redaction test has a target, and that string is planted precisely because Google's real `INVALID_ARGUMENT` text does **not** echo the key back (unlike OpenAI's 401) — the redactor is defence in depth here rather than a response to observed leakage, which is why this one fixture cannot be recorded. The 429 stays synthesized because provoking a real quota rejection would mean sustained abuse of the live endpoint.
* **Binding on 20.6:** a `synthesized/` fixture may assert only what its hand-written nature can support. If an assertion is a claim about the provider's wire, it belongs on a `recorded/` fixture — and if the recorder already makes a call whose response body carries that shape, record the body rather than describing it in a `_comment`.
...
* **The fixture's non-normalization is load-bearing and is asserted as such.** The recorded 768-wide vectors measure L2 ≈ 0.585. A dedicated test asserts `abs(magnitude - 1.0) > 0.01` on every entry *before* the normalization tests run, because if Google ever starts pre-normalizing truncated output, both Decision-#7 assertions would start passing vacuously and the suite would go green on a dead test rather than red on a changed premise.

**Deviations from the checklist, with justification:**

* **[structural, documented] `to_batch_body/2` returns `{:ok, map()} | {:error, EmbeddingAdapterError.t()}`, not the bare `map()` its OpenAI counterpart returns.** The nil-model rejection has nowhere else to live: `:model` is required *per sub-request* and again in the URL path, so the body builder is the component that discovers it is absent, and hoisting a `gate_model/2` into `run_gates/2` would mean two places deciding what a legal model is. This aligns with the Gemini *provider* family — `Gemini.Images.to_image_request_body/2` is likewise tuple-returning — at the cost of aligning with the embeddings *capability* family. Recorded in the adapter's naming-parity block. The design's 20.5.1 bullet ("`to_batch_body/2` with `request.model == nil` returns `:invalid_request` before any HTTP I/O") reads naturally under this shape and was the tiebreaker.

  **Binding on 20.6, because this helper now sits at the intersection of two families that point different ways.** `CLAUDE.md` adjudicates *names* ("alignment governs names, not visibility or arity") and is silent on *return shapes* — and a return-shape divergence is strictly stronger, since a name costs a reader one lookup while a return type changes every call site's `with` chain. The rule this phase implies: **the capability family wins by default; the provider family wins only when a per-provider invariant forces it, and the forcing invariant is named in the helper's `@doc false` block.** 20.5 satisfies that test (Gemini needs `model` per sub-request *and* in the URL path, so the body builder is genuinely the component that discovers it is absent). `ALLM.Providers.Voyage.Embeddings` has **no** `Voyage.Images` sibling and therefore only one axis: it returns a bare `map()` from `to_json_body/2`, matching `OpenAI.Embeddings`, unless a Voyage-specific invariant forces otherwise — otherwise Gemini is left the sole tuple-returner in a three-member family with the divergence documented in only one of the three files. Phase Module Trees should list seams with their **return types**, not just name/arity, so this conflict surfaces at design time rather than at implementation time.
* **[structural, documented] `translate_reason/1` ships as a `@doc false` seam rather than a `defp`.** `ALLM.Providers.Gemini.classify_error/3` is owned by the chat adapter and its `AdapterError` enum is *wider* than `EmbeddingAdapterError`'s — it carries `:content_filter`, which would raise `ArgumentError` out of `EmbeddingAdapterError.new/2` and violate behaviour invariant 2 ("never raises for HTTP-shaped failures"). Today's classifier cannot emit it, so the guard is unreachable from any test that drives the real classifier; exposing the seam is what lets the guard be bound directly instead of shipping as an uncovered defensive line in the one function whose job is to not raise. Not in the design's seam list; added. *(Amended during the 20.5 fix step: the seam shipped with **no test calling it** — `grep -rn "translate_reason" test/` returned nothing — so the cost of the widened public surface was paid and the benefit was not collected, and the module carried a public function for an uncovered defensive line anyway. A `describe "translate_reason/1"` block now exists, and its load-bearing member is not the nine-row pass-through table but the subset property `@translatable_reasons ⊆ EmbeddingAdapterError.legal_reasons/0`, asserted by translating **every** `AdapterError.legal_reasons/0` member and requiring the result to be legal. That is what a per-reason table cannot do: adding `:content_filter` to `@translatable_reasons` is a plausible future edit — it IS a legal `AdapterError` reason — and would violate invariant 2 with an otherwise-green suite. A companion test refutes `AdapterError`'s enum being a subset of the embeddings one, so the subset assertion cannot go vacuous if the enums converge.)* **Binding on 20.6:** promote a reason translator to a `@doc false` seam **only alongside that subset test**; without it, leave it `defp`. A seam justified by testability and then not tested is strictly worse than the `defp` it replaced.
* **[structural, documented] `redact_key_material/1` reads the message off the RAW body via a new `provider_message/2`, not off `chat_error.message`.** `ALLM.Error.AdapterError` types `:message` as `String.t()`, but `classify_error/3` populates it with `Map.get(error, "message", default)` straight off the decoded body — so `{"error": {"message": 123}}` puts a non-binary there and the declared type is optimistic. Dialyzer believes the declaration and proved the non-binary arm unreachable (`pattern_match_cov`, then `guard_fail` when the same branch was rewritten as an `if`). Re-sourcing from the body both restores the arm and is the more honest seam: redaction is a property of untrusted provider text, not of a typed struct field. The underlying type lie in `Gemini.classify_error/3` is shipped v0.4 code outside this Module Tree — raised, not fixed, per cross-phase bug discipline.
* **[tactical] `base_url/1` falls through to `@base_url` for a non-binary `adapter_opts[:endpoint]`** where `Gemini.Images.endpoint_url/2` (`lib/allm/providers/gemini/images.ex:503-511`) would raise `CaseClauseError`. A `CaseClauseError` out of an adapter is exactly the invariant-2 violation `dispatch_chunk/2` turns into an `ArgumentError` two layers up. Pinned by a test.
* **[tactical] The `models/` prefix is applied idempotently in BOTH directions** — `prefix_model/1` for the body, `strip_model_prefix/1` for the URL path — so a caller who passes the fully-qualified `"models/gemini-embedding-001"` gets neither `models/models/…` on the wire nor a doubled path segment. Only the body direction was in the Test Plan; the URL direction has its own test.
* **[tactical] `decode_response/4` ignores its `headers` argument.** Kept in the signature for family alignment; the moduledoc and the naming-parity block both say why there is nothing to read.
* **[tactical] A non-binary input ELEMENT is rejected by a third pre-flight gate, `gate_input/2`.** *(Amended during the 20.5 fix step — the original text said `materialize_text/1` "stringifies a non-binary input element rather than rejecting it", justified as "producing a legal body beats producing a `Protocol.UndefinedError` from `Jason`". That justification was **inverted**: `to_string/1` raises `Protocol.UndefinedError` itself, just earlier and from the adapter rather than the encoder, so `embed/2` raised on `input: [%{"a" => 1}]` — a behaviour-invariant-2 violation in the one adapter that defends the container shape explicitly for that very reason, and one 20.3's `ALLM.EmbeddingBatch.dispatch_chunk/2` re-frames as an `ArgumentError` two layers up. The OpenAI sibling does not raise on the same input: `to_json_body/2` passes `request.input` through verbatim, Jason encodes the map, and the provider 400s into `{:error, _}`.)* `materialize_text/1` is gone; `sub_request/3` passes the element through verbatim like the OpenAI sibling, and `gate_input/2` runs third in `run_gates/2` returning `:invalid_request` with `metadata: %{field: :input}` when any element is non-binary. **Binding on 20.6** under the same `gate_*/2` prefix if its body builder likewise reaches inside elements: the container gate and the element gate are two different defences and 20.4's carried-forward fix only supplied the first.

**Carried-forward items honoured, not re-litigated:**

* **The `:timeout` 9-vs-3 retry-budget quirk was copied as-is.** Two nested `ALLM.Retry.run/3` loops, identical in shape to `openai/embeddings.ex` and both image adapters. The adapter's `## Retry integration` moduledoc states the exception explicitly rather than claiming the façade's widened list decides. Tracked in `ASKS.md`; a per-adapter "fix" would have made the three embeddings adapters diverge from each other.
* **Alias ordering:** the brace group `ALLM.{Embedding, …}` sorts **before** `ALLM.Error.EmbeddingAdapterError` — the opposite of the image modules. Credo's `AliasOrder` enforces it.
* **Fixture-relative assertions throughout.** No test asserts a literal dimensionality; widths come from `dimension_of/1` and the reduced/batch comparison is the *relation* `dimension_of(reduced) < dimension_of(batch)`. A re-record at a different width costs zero test edits.
* **Doctests are wired.** `doctest Embeddings` is declared in `embeddings_test.exs`; all four `iex>` examples are hermetic (two drive `adapter_opts[:embedding_script]` and the empty-input gate, one passes a literal `api_key:`).
* **The `request_timeout` test ships as the corrected two-assertion split** — a `Req.Test.transport_error(conn, :timeout)` test for the conversion, plus a `prepare_request/2` assertion that the opt lands as `:receive_timeout`, with a comment naming the shared `build_request/3` that makes the second bind `embed/2` too. No test passes a `request_timeout:` an in-process plug cannot consult.
* **Security re-check, both items.** `to_embedding_adapter_error/4`'s metadata is exactly `[:google_status, :status]` (+ `:request_id`) — structural, never a body preview; `:cause` is never populated from a classified HTTP error and `Jason.DecodeError`'s `:data` is blanked by `sanitize_cause/1`; the redactor is widened to Google's `AIza…` and `ya29.…` shapes rather than inheriting OpenAI's prefixes, with a test asserting neither `inspect/1` nor `Jason.encode!/1` of the struct contains the fixture's key-shaped string. The `embedding_script` short-circuit keys on the per-call opt and nothing else, pinned by a test that shows the real path is taken (and raises on the missing key) without it.
* **`README.md` untouched**, per its absence from this Module Tree.

**Not done here:** `ALLM_PROVIDER=gemini mix run examples/16_embed_single.exs` — the script lands in 20.7 and does not exist yet.

**Files created:** `lib/allm/providers/gemini/embeddings.ex`; `test/allm/providers/gemini/embeddings_test.exs`, `embeddings_wire_test.exs`, `embeddings_conformance_test.exs`; `scripts/record_gemini_embeddings_fixtures.exs`; five fixtures under `test/fixtures/gemini/embeddings/` (three `recorded/`, two `synthesized/`).
**Files modified:** `test/support/gemini_fixtures.ex` (`embeddings_recorded/1`, `embeddings_synthesized/1`), `mix.exs` (`groups_for_modules` → `Providers`, forced by the committed audit gate), this document.
...
- [x] Every amendment block opens with `> **Phase 20 amendment (commits <first-sha>..<last-sha>).**` per agent-spec/DESIGN.md rule 21
