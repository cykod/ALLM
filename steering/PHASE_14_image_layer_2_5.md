...
- **Non-obvious decisions:**
  ...
  9. **Retry policy reuses v0.2's `default_policy/0` for backoff timing; image-side call site augments `retry_on` with the four image-error atoms.** Resolves Phase 4 key decision (b). The chat-side `default_policy/0` (`max_attempts: 3`, `base_delay_ms: 500`, `max_delay_ms: 8000`, `jitter_ms: 250`, `respect_retry_after: true`) is reused unchanged AT ITS SOURCE — no second default policy is introduced. However, `default_policy/0.retry_on` is `[429, 500, 502, 503, 504, :timeout]` (HTTP status codes plus one atom), and image adapters surface CLOSED-ENUM ATOMS from the §35.4 / Phase 14.1 `ImageAdapterError` reason set — never HTTP status integers. The `lib/allm.ex` image façade therefore augments the materialised policy at each call site via `augment_image_retry_policy/1`, appending `[:rate_limited, :provider_unavailable, :timeout, :network_error]` to `retry_on` (idempotent via `Enum.uniq/1`; `:timeout` overlaps with the chat-side default). The augmentation is purely additive: HTTP-coded reasons in the chat-side default remain in the materialised list (defensive for future image adapters that surface raw status codes). Image generation calls have longer wall-clock latency than chat (gpt-image-1 calls run 5-30s vs chat sub-5s), but the retry behaviour is governed by the FAILURE-mode latency, not the success-path latency: 429s have `Retry-After` hints (which `respect_retry_after: true` honors) and 5xx errors return immediately. Per-image retry tuning remains callsite-overridable via `engine.retry: [base_delay_ms: 2_000]`. *Docs target: `@moduledoc ALLM.Retry` (extend with image-side note + call-site augmentation pattern).*
