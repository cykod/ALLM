...
### Wire-field map (per AGENT_DESIGN_SPEC.md rule 15)
...
- [ ] Reviewed via `/review` per `AGENT_REVIEW_SPEC.md` — BLOCKING examples gate (`OPENAI_API_KEY=sk-... mix run examples/run_all.exs` exit 0; `ALLM_PROVIDER=anthropic mix run examples/run_all.exs` exit 0 with `[SKIP]` for `10_*.exs`).
