...
| Concern | Wire | Status |
|---------|------|--------|
| Correlation header | **none** (no `x-request-id`/`x-goog-request-id`); body `responseId` → `response.id` | CONFIRMED, consistent with Phase 20's Gemini finding |
...
| STT inline limit | 20 MB **request** (Gemini docs), which the base64-encoded audio counts against → `max_audio_bytes/0 == div((20 * 1024 * 1024 - 64 * 1024) * 3, 4)` (raw bytes whose encoding plus prompt headroom fits) | **inferred** → probe arm (25.5.2). A raw cap of `20 * 1024 * 1024` would encode to ~26.7 MiB and let 15–20 MB calls reach a provider 400 instead of the local gate |
| STT accepted mimes | `audio/wav, audio/mpeg (mp3), audio/aiff, audio/aac, audio/ogg, audio/flac` | **inferred** (docs) → adapter gate; one probe arm per clip format the 25.4 recorder produces. `nil` mime → `:invalid_request` (Gemini requires `mimeType`). OpenAI's `opus` output is Ogg-encapsulated and `Audio.from_file("x.opus")` yields `audio/opus`, which is not in the set: the probe sends the `.opus` clip as both `audio/opus` and `audio/ogg`, and if only `audio/ogg` is accepted the adapter keeps a one-row alias table (`audio/opus` → `audio/ogg` on the wire) rather than rejecting it |
