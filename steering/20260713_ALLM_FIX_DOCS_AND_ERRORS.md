...
- `stream/3 path carries max_tokens identically to chat/3` — matrix-identical assertion on `do_stream_step_call/3`'s built request (streaming ≡ non-streaming for param coverage, per `agent-spec/DESIGN.md` rule 10).
...
- **Stream-equivalence (Phase 4):** the `max_tokens`-coverage assertions are matrix-identical for `chat/3` (non-streaming) and `stream/3` (streaming) per `agent-spec/DESIGN.md` rule 10 — one row per path, no relaxations (both consume the same `build_request/4`).
