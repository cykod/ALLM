...
7. **Conformance-suite case counts are part of the contract.** Add `@case_count N` + `case_count/0` introspection + a meta-test asserting the injected `describe/2` produces exactly `@case_count` `test` cases. Adding a case without bumping the attribute is silent drift; the attribute makes it a loud PR-review signal.
